...
function countryToRecord(country) {
  ...
  return {
    ...
    primaryCallNumber: country.primaryCallNumber || '',
    secondaryCallNumber: country.secondaryCallNumber || '',
...
export const persistCountries = async list => {
  for (const c of list) {
    ...
    const body = {
      ...
      // Every save re-persists the whole list (see persistLocations above) —
      // an untouched sibling record with no number yet must omit the field
      // (PartialType/UpdateCountryDto leaves it alone) rather than resend an
      // empty string, which CreateCountryDto's required-format validator
      // rejects even via the update path.
      primaryCallNumber: c.primaryCallNumber || undefined,
      secondaryCallNumber: c.secondaryCallNumber || undefined,
...
export async function fetchAll() {
  const [verticals, categories, services, doctors, locations, countries, vouchers, reviews, footer, global, pages] =
    await Promise.all([
  ...
  return {
    services, verticals, categories, doctors, locations, countries, vouchers, reviews, pages,
    site: { footer, global },
  }
}
