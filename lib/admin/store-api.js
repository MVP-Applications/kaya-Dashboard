...
function treatmentToService(treatment) {
  ...
  return {
    ...
    durationMins: treatment.duration ?? treatment.durationMins ?? '',
    sessions: treatment.sessions ?? '',
    downtimeNotes: treatment.downtimeNotes || '',
    downtimeLevel: treatment.downtimeSeverity || '',
...
export const persistVerticals = async list => {
  for (const v of list) {
    ...
    const body = { slug, translations, color: v.color || '', hintText: v.hint || '' }
...
export const persistServices = async list => {
  for (const s of list) {
    ...
    const body = {
      ...
      imageUrl,
      icon: s.thumb,
      duration: s.durationMins === '' || s.durationMins == null
        ? undefined
        : String(s.durationMins),
      sessions: s.sessions,
      downtimeNotes: s.downtimeNotes,
      downtimeSeverity: s.downtimeLevel,
      isPublished: true,
      translations,
    }
