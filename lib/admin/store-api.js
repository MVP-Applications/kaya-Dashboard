 ...
 *
 * Services/Verticals (Treatments/Pillars, KA-24), Doctors (KA-25) and
 * Locations/Clinics (KA-26) are connected to kaya-nest-api. Everything else
...
export async function fetchAll() {
  const [verticals, services, doctors, locations] = await Promise.all([
    fetchVerticals(), fetchServices(), fetchDoctors(), fetchLocations(),
  ])
  return {
    services, verticals, doctors, locations,
    reviews: [], vouchers: [],
    pages: {}, site: {},
  }
}
