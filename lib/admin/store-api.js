  ...
  for (const s of list) {
    ...
    if (s.nameAr && s.whatAr && s.mechanismAr) {
      translations.push({
        ...
        benefits: (s.benefitsAr || []).filter(Boolean),
      })
    }
    const body = {
      slug: s.slug,
...
async function fetchPages() {
  ...
  for (const [feId, cfg] of Object.entries(PAGE_SECTIONS)) {
    ...
    for (const [sectionId, sectionCfg] of Object.entries(cfg.sections)) {
      ...
      out[sectionId] = en ? mapSection(en.content, sectionCfg.fields, 'toDashboard') : {}
    }
    result[feId] = out
  }
  return result
...
export async function persistPageSection(pageId, sectionId, sectionValue) {
  ...
  const content = mapSection(sectionValue, cfg.fields, 'toContent')
  await apiRequest(ApiEndpoints.pages.adminSections(backendPageId), {
    method: 'PUT',
    body: { sections: [{ key: cfg.key, translations: [{ locale: 'EN', content }] }] },
  })
}
