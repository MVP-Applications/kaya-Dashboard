...
// Turns the backend's flat per-benefit string back into the structured
// icon+title+description shape ServiceForm edits (KA-30) — `d` is the only
// part that ever round-trips to the backend; `i`/`t` start blank on load.
function benefitsToForm(list) {
  return (list || []).map(d => ({ i: '', t: '', d }))
}
...
function treatmentToService(treatment) {
  ...
  return {
    ...
    // `image`/`thumb`/`category`/`sub`/`suitable`/`downtimeLevel` have no
    // backend column (KA-30) — always blank on load, edited locally only.
    image: '',
    thumb: '',
    category: '',
...
export const persistVerticals = async list => {
  for (const v of list) {
    ...
    const translations = [{ locale: 'EN', name: v.label }]
    if (v.labelAr) translations.push({ locale: 'AR', name: v.labelAr })
    const body = { slug, translations }
...
export const persistServices = async list => {
  for (const s of list) {
    ...
    // Only the description half of each structured benefit (KA-30's `i`/`t`
    // have nowhere to go) round-trips to the backend's flat string list.
    const benefitStrings = list => (list || []).map(b => b.d).filter(Boolean)
    ...
    const translations = [{
      ...
      benefits: benefitStrings(s.benefits),
    }]
    if (s.nameAr && s.whatAr && s.mechanismAr) {
      translations.push({
        ...
        benefits: benefitStrings(s.benefitsAr),
      })
    }
    const body = {
      slug: s.slug,
      ...
      isPublished: true,
      translations,
      // `image`/`thumb`/`category`/`sub`/`suitable`/`downtimeLevel` are
      // deliberately left out — the backend has no column for them (KA-30)
      // and rejects any request body with an unrecognized property.
    }
...
export async function fetchAll() {
  const [verticals, services, doctors, locations, vouchers, reviews, footer, global, pages] = await Promise.all([
    fetchVerticals(), fetchServices(), fetchDoctors(), fetchLocations(), fetchVouchers(), fetchReviews(),
    fetchSiteGroup('footer'), fetchSiteGroup('global'), fetchPages(),
  ])
  return {
    services, verticals, doctors, locations, vouchers, reviews, pages,
    site: { footer, global },
  }
}
