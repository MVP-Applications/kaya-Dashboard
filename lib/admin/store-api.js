...
// ── Analytics ─────────────────────────────────────────────────────────────
//
// No aggregate endpoint exists yet. It should return the same shape as
// buildSampleAnalytics() in analytics-sample.js — raw requests, voucher orders
// and daily visits — or, better, pre-aggregated rows once volumes grow.
export const fetchAnalyticsData = () => notConnected('Analytics')
...
// ── Blog posts ────────────────────────────────────────────────────────────
//
// kaya-nest-api has no Blog module yet. Everything below is written against
// the routes in ApiEndpoints.blogs and the backend shape this app's other
// modules already use (translations[] per locale, *Url for media, an
// uppercase status enum), so connecting it should mean:
//
//   1. flip BLOGS_API_READY to true, and
//   2. adjust blogToRecord / recordToBlogBody if the real field names differ.
//
// Nothing above this section — AdminContext, BlogsView, BlogForm — needs to
// change. Until then every call fails with the usual "isn't connected" error,
// which the Blog screen shows in place of the list.

const BLOGS_API_READY = false

const BLOG_STATUS_TO_BACKEND = { draft: 'DRAFT', published: 'PUBLISHED' }
const BLOG_STATUS_FROM_BACKEND = { DRAFT: 'draft', PUBLISHED: 'published' }

const blogIds = new Map() // client id -> backend id
// Last-saved body per post, so a list save only sends the posts that changed
// instead of re-PUTting every one (AdminContext always persists the list).
const blogSaved = new Map()

function blogToRecord(b) {
  blogIds.set(b.id, b.id)
  ...
  const record = {
    ...
    image: b.imageUrl || '',
    writer: b.author || '',
    status: BLOG_STATUS_FROM_BACKEND[b.status] || 'draft',
...
function recordToBlogBody(r, imageUrl) {
  const translations = [{ locale: 'EN', title: r.title, content: r.content }]
  if (r.titleAr && r.contentAr) translations.push({ locale: 'AR', title: r.titleAr, content: r.contentAr })
  return {
    slug: r.slug,
    imageUrl,
    author: r.writer,
    status: BLOG_STATUS_TO_BACKEND[r.status] || 'DRAFT',
...
export async function fetchBlogs() {
  if (!BLOGS_API_READY) return notConnected('Blog posts')
...
export const persistBlogs = async list => {
  if (!BLOGS_API_READY) return notConnected('Saving blog posts')
  for (const r of list) {
    const existingId = blogIds.get(r.id)
    if (existingId && blogSaved.get(r.id) === JSON.stringify(r)) continue

    const imageUrl = await uploadImageIfNeeded(r.image)
    const body = recordToBlogBody(r, imageUrl)
    const { data } = existingId
...
export const removeBlog = async id => {
  if (!BLOGS_API_READY) return notConnected('Deleting blog posts')
...
//
// A single document — GET/PUT the whole thing, like Footer & Global. No
// backend module exists yet; the document's shape is the contract, written
// up for both the backend and website teams in docs/tell-us-everything.md.
// It is sent and received as-is, so connecting it should only mean flipping
// TELL_US_API_READY (and mapping here if the backend reshapes it).

const TELL_US_API_READY = false

export async function fetchTellUs() {
  if (!TELL_US_API_READY) return notConnected('Tell Us Everything')
  const { data } = await apiRequest(ApiEndpoints.tellUs.admin)
  return data
}

export async function persistTellUs(doc) {
  if (!TELL_US_API_READY) return notConnected('Saving Tell Us Everything')
  await apiRequest(ApiEndpoints.tellUs.admin, { method: 'PUT', body: doc })
}

// ── Page builder (custom pages + fixed-page visibility) ───────────────────
//
// Pages the admin builds from blocks, and a visible/hidden flag for the fixed
// pages. Neither exists on the backend yet, and both are kept apart from the
// connected pages module above so nothing there changes. The document shape
// is the contract — see docs/page-builder.md. Pages are sent and received
// as-is, so connecting should mean flipping PAGE_BUILDER_API_READY (and
// mapping here if the backend reshapes them).

const PAGE_BUILDER_API_READY = false
...
export const persistCustomPages = async list => {
  ...
  for (const p of list) {
    ...
    const { id: _clientId, ...rest } = p
    const body = { ...rest, blocks }
    const { data } = existingId
...
export const removeCustomPage = async id => {
  if (!PAGE_BUILDER_API_READY) return notConnected('Deleting pages')
...
export async function fetchPageVisibility() {
  if (!PAGE_BUILDER_API_READY) return notConnected('Page visibility')
  const { data } = await apiRequest(ApiEndpoints.customPages.adminVisibility)
  return data || {}
}

export async function persistPageVisibility(map) {
  if (!PAGE_BUILDER_API_READY) return notConnected('Hiding pages')
  await apiRequest(ApiEndpoints.customPages.adminVisibility, { method: 'PUT', body: map })
}
...
const CUSTOMERS_API_READY = false
...
function leadToRequest(lead) {
  return {
    ...
    // Tell Us Everything answers — not on the backend's Lead yet. Read only if
    // present, so nothing changes until it is (docs/tell-us-everything.md).
    answers: Array.isArray(lead.answers) ? lead.answers : [],
    notes: lead.notes || '',
  }
}

// Whether GET /admin/leads (and its CSV export) accepts a `treatmentId`
// filter. Unconfirmed, so it stays off: sending a parameter the backend
// doesn't whitelist could fail the whole Requests list. While off, picking a
// treatment shows a clear "isn't connected" error instead of silently
// returning unfiltered results — and without one, the request is unchanged.
const LEADS_TREATMENT_FILTER_READY = false

function requestQueryParams({ search, source, status, country, city, treatment, from, to, page, pageSize } = {}) {
  const params = new URLSearchParams()
  if (treatment) {
    if (!LEADS_TREATMENT_FILTER_READY) notConnected('Filtering enquiries by treatment')
    params.set('treatmentId', treatmentIds.get(treatment) || treatment)
  }
  if (search) params.set('search', search)
