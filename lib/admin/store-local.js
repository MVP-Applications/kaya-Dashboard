...
import {
  ...
  seedRequests, seedVoucherRequests, seedLocations, seedPages, seedSite, seedUsers, seedBlogs, seedCustomers,
} from './demo-seed'
import { REQUEST_STATUS_LABELS, REQUEST_SOURCE_LABELS } from './seed'
import { toCsv } from './csv'
import { buildSampleAnalytics } from './analytics-sample'
import { seedTellUs } from './tell-us'
import { CUSTOM_PAGES } from '@/lib/seed-data/custom-pages'
...
const KEYS = {
  ...
  pageVisibility: 'kaya_admin_page_visibility_v1',
...
/**
 * Blog posts load on their own, outside fetchAll, so that on the real
 * backend — where blogs aren't connected yet — a failure here can't take the
 * rest of the catalogue down with it. See fetchBlogs in store-api.js.
 */
...
/**
 * The Tell Us Everything questionnaire — one document, not a list. Loaded on
 * its own like blog posts, for the same reason (see fetchTellUs in store-api.js).
 */
...
/**
 * Pages built in the page builder, and the visible/hidden flag for the fixed
 * pages. Both load on their own, like blog posts (see store-api.js).
 */
export async function fetchCustomPages() {
  return settle(load('customPages', () => JSON.parse(JSON.stringify(CUSTOM_PAGES))))
}

/** { [fixedPageId]: false } for each hidden fixed page; absent means visible. */
export async function fetchPageVisibility() {
  const stored = read(KEYS.pageVisibility, null)
  return settle(stored && typeof stored === 'object' && !Array.isArray(stored) ? stored : {})
}

export async function persistPageVisibility(map) {
  write(KEYS.pageVisibility, map)
}
...
// `treatmentName` is the display name for the `treatment` slug filter —
// enquiries record the name, not the slug. Resolved once in filteredRequests.
function matchesRequestFilters(r, { search, source, status, country, city, treatmentName, from, to } = {}) {
  if (status && r.status !== status) return false
  if (treatmentName && (r.treatment || '').toLowerCase() !== treatmentName.toLowerCase()) return false
...
function filteredRequests(filters = {}) {
  const treatmentName = filters.treatment
    ? load('services', seedServices).find(s => s.slug === filters.treatment)?.name || filters.treatment
    : ''
  return load('requests', seedRequests)
    .filter(r => matchesRequestFilters(r, { ...filters, treatmentName }))
    .slice()
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
}
...
/**
 * Everything the Analytics screen aggregates: sample history built from the
 * catalogue in this browser, plus the preview inbox itself (see
 * analytics-sample.js). Rebuilt on each call, never stored.
 */
export async function fetchAnalyticsData() {
  return settle(buildSampleAnalytics({
    services: load('services', seedServices),
    verticals: load('verticals', seedVerticals),
    doctors: load('doctors', seedDoctors),
    vouchers: load('vouchers', seedVouchers),
    ...
    voucherRequests: load('voucherRequests', seedVoucherRequests),
  }))
}
