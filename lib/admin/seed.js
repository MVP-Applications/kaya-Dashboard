 ...
 *
 * `image`, `thumb`, `category`, `sub`, `suitable`, and `downtimeLevel` (plus
 * the `i`/`t` on each benefit) have no column on kaya-nest-api's Treatment
 * model (KA-30) — restored to the form anyway per KA-30, but `persistServices`
 * (lib/admin/store-api.js) still only sends the fields the backend actually
 * has, so these are edited locally and never make it past a save.
 */
