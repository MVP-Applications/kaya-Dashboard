...
import { CUSTOMERS } from '@/lib/seed-data/customers'
...
export function seedRequests() {
  const hoursAgo = h => new Date(Date.now() - h * 3600_000).toISOString()
  return [
    // A full Tell Us Everything journey: area → shared questions → concerns →
    // a suggested treatment picked from the results, then booked.
    {
      id: 'req-1043', source: 'concern', status: 'new',
      name: 'Noura Al Hashimi', mobile: '+971 56 000 0009', email: 'noura@example.com',
      gender: 'Female', country: 'UAE', city: 'Dubai',
      treatmentArea: 'Hair', treatment: 'PRP Hair Restoration', doctor: '',
      concerns: ['Thinning Hair', 'Hair Loss or Receding Hairline'],
      answers: [
        ...
        { questionId: 'concerns-hair', question: 'What specifically bothers you?', answers: ['Thinning Hair', 'Hair Loss or Receding Hairline'] },
      ],
      message: 'Noticed more shedding since having my baby last year.',
      createdAt: hoursAgo(1), notes: '',
    },
    {
    ...
    },
    {
      ...
      // Tell Us Everything answers, as the website sends them (see docs/tell-us-everything.md).
      answers: [
        { questionId: 'age', question: 'Which age range are you in?', answers: ["In your 30's"] },
        { questionId: 'concerns-mens', question: 'What specifically bothers you?', answers: ['Hair Loss or Thinning'] },
      ],
    ...
    },
    {
      ...
      answers: [
        { questionId: 'gender', question: 'What is your gender?', answers: ['Female'] },
        { questionId: 'age', question: 'Which age range are you in?', answers: ["In your 40's"] },
        { questionId: 'concerns-wellness', question: 'What specifically bothers you?', answers: ['Fatigue & Low Energy', 'Biological Ageing'] },
      ],
    ...
    },
    {
      ...
      answers: [
        { questionId: 'gender', question: 'What is your gender?', answers: ['Male'] },
        { questionId: 'age', question: 'Which age range are you in?', answers: ["In your 30's"] },
        { questionId: 'concerns-body', question: 'What specifically bothers you?', answers: ['Stubborn Fat', 'Body Reshaping'] },
      ],
...
/** Sample website accounts — relative days become real timestamps at load time. */
export function seedCustomers() {
  const daysAgo = d => (d == null ? null : new Date(Date.now() - d * 86400_000).toISOString())
  return CUSTOMERS.map(({ joinedDaysAgo, lastSeenDaysAgo, healthConsentDaysAgo, ...c }) => ({
    ...c,
    createdAt: daysAgo(joinedDaysAgo),
    lastSignInAt: daysAgo(lastSeenDaysAgo),
    healthConsentAt: daysAgo(healthConsentDaysAgo),
  }))
}
